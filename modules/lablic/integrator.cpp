...
vec2 Integrator::sampleFromField(const Volume* vol, size3_t dims, const vec2& position, bool isNormalized = false)
{
...
vec2 Integrator::euler(const Volume* vol, size3_t dims, const vec2& position, float stepSize)
{
    vec2 nextVec =  sampleFromField(vol, dims, position); 
